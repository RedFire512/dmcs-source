...
void CDMCBot::ResetNavigationParams()
{
...
void CDMCBot::AddWaypoint( Vector center, NavTraverseType transient, int attribute, int id, bool AddToTail)
{
	NavAreaData_t data;
...
void CDMCBot::DealWithObstacles( CBaseEntity *pTouchEnt, CUserCmd &cmd )
{
	if( !pTouchEnt )
		return;
...
void CDMCBot::AddRandomPath( float randomStartAngle )
{
...
bool CDMCBot::ArrivedToWaypoint()
{
...
void CDMCBot::CheckStuck( CUserCmd &cmd )
{
...
void CDMCBot::CheckNavMeshAttrib( trace_t *ptr, CUserCmd &cmd )
{
...
bool CDMCBot::CreatePath( CBasePlayer *pPlayer, Vector OptionalOrg )
{
...
bool CDMCBot::CreateHidePath( Vector &HiDeSpot )
{
	//DevMsg("Let's hide\n");
...
void CDMCBot::SelectSchedule( bool forcePath )
{
	Vector HideSpot;
...
bool CDMCBot::SafePathAhead( Vector origin )
{
...
void CDMCBot::Navigation( CUserCmd &cmd )
{
	int iLikelihood = 3;
	if (m_bEnemyOnSights)
		iLikelihood = 1;
